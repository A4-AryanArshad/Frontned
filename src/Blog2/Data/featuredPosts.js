const featuredPosts = [
    {
      image: "./Data/assets/images/featured-1.jpg",
      title: "New technology is not good or evil in and of itself",
      description:
        "Vestibulum vehicula dui venenatis neque tempor, accumsan iaculis sapien ornare. Sed at ante porta, ullamcorper massa eu, ullamcorper sapien...",
      tags: ["Design", "Idea", "Review"],
      authors: [
        "./Data/assets/images/author-1.jpg",
        "./Data/assets/images/author-2.jpg",
      ],
    },
    {
      image: "./Data/assets/images/featured-2.jpg",
      title: "It’s a new era in design, there are no rules",
      description:
        "Quibus autem in rebus tanta obscuratio non fit, fieri tamen potest, ut id ipsum, quod interest, non sit magnum...",
      tags: ["Creative", "Product"],
      authors: ["./Data/assets/images/author-3.jpg"],
    },
    {
      image: "./Data/assets/images/featured-3.jpg",
      title: "Perfection has to do with the end product",
      description:
        "Aenean eget urna aliquet, viverra orci quis, aliquam erat. Ut rutrum quam quam, eu eleifend est blandit et...",
      tags: ["Design", "Creative", "Idea"],
      authors: ["./Data/assets/images/author-4.jpg"],
    },
    {
      image: "./Data/assets/images/featured-4.jpg",
      title: "Everyone has a different life story",
      description:
        "Non est igitur summum malum dolor. Tu autem inter haec tantam multitudinem hominum interiectam non vides nec laetantium...",
      tags: ["People", "Story"],
      authors: [
        "./Data/assets/images/author-5.jpg",
        "./Data/assets/images/author-2.jpg",
      ],
    },
    {
      image: "./Data/assets/images/featured-5.jpg",
      title: "The difference is quality",
      description:
        "Vide, ne etiam menses! nisi forte eum dicis, qui, simul atque arripuit, interficit. Atque his de rebus et splendida est...",
      tags: ["Design", "Lifestyle", "Idea"],
      authors: ["./Data/assets/images/author-6.jpg"],
    },
    {
      image: "./Data/assets/images/featured-6.jpg",
      title: "Problems are not stop signs, they are guidelines",
      description:
        "Quid ad utilitatem tantae pecuniae. Duo enim genera quae erant, fecit tria. Et quod est munus, quod opus sapientiae...",
      tags: ["Idea", "Creating"],
      authors: ["./Data/assets/images/author-3.jpg"],
    },
  ];
  
  export default featuredPosts;
  